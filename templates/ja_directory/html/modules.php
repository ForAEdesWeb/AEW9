...
{ 
	...
	$fullWidth			= $params->get('full-width',1);
...
{ 
	...
	if (!empty ($module->content)) {
		...
		$html .= "<{$moduleTag} class=\"section {$moduleClass}\" id=\"section-{$module->id}\">" .
					"<div class=\"section-inner {$moduleClassSfx}\">" . $badge;

		if ($module->showtitle != 0) {
			$html .= "<{$headerTag} class=\"section-title {$headerClass}\"><span>{$module->title}</span>";

			if($moduleIntro) {
				$html .= "<p class=\"container-sm section-intro hidden-xs\"><span>{$moduleIntro}</span></p>";
			}
