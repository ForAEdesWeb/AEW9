 ...
 * @subpackage  com_modulesmanagerck
 * @since       1.6
 */
class ModulesmanagerckModelModule extends JModelAdmin
{
	/**
	 * @var    string  The prefix to use with controller messages.
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_MODULESMANAGERCK';
	...
	protected function populateState()
	{
		...
		if (!$pk)
		{
			if ($extensionId = (int) $app->getUserState('com_modulesmanagerck.add.module.extension_id'))
			{
		...
		// Load the parameters.
		$params	= JComponentHelper::getParams('com_modulesmanagerck');
	...
	public function delete(&$pks)
	{
		...
		foreach ($pks as $i => $pk)
		{
			...
			{
				// Access checks.
				if (!$user->authorise('core.delete', 'com_modulesmanagerck'))
				{
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_modulesmanagerck.edit.module.data', array());
		...
		if (empty($data))
		{
			...
			$params = $app->getUserState('com_modulesmanagerck.add.module.params');
			if (is_array($params))
			{
	...
	public function getItem($pk = null)
	{
		...
		if (!isset($this->_cache[$pk]))
		{
			...
			if (empty($pk))
			{
				...
				{
					$app = JFactory::getApplication();
					$app->redirect(JRoute::_('index.php?option=com_modulesmanagerck&view=modules', false));
					return false;
				}
			}
	...
	*/
	...
	public function save($data)
	{
		...
		$dispatcher->trigger('onExtensionAfterSave', array('com_modulesmanagerck.module', &$table, $isNew));
	...
	{
		parent::cleanCache('com_modulesmanagerck', $this->getClient());
	}

	/**
	 * Method to save the module title
	 *
	 * @access	public
	 * @return	boolean	True on success
	 */
	...
	/**
	 * Method to save the module position
	 *
	 * @access	public
	 * @return	boolean	True on success
	 */
	function saveposition($id, $position)
	{
		...
		$rowtest = $this->getTable();
		$rowtest->load( (int) $id );

		return $rowtest->position;
	}

	function updatemodule($data) {
		$data = $this->getTable('Module');
	}
	 ...
	 * @return	boolean	True on success
	 */
	public function addmodule($extensionid, $position, $ordering, $type)
	{
	// var_dump ($this->getItem(0));
			// die();
		$row = $this->getTable('Extension');
		// $item = $this->getItem();
		// var_dump($item);die();
		// update position
		$row->load( (int) $extensionid );

		if ($extensionid)
		{

			$data = $this->getTable('Module');
			$data->title = JText::_('NEW MODULE');
			...
			$data->language = '*';
			$data->module = $type;
			$data->params = $this->convertModuleParams($this->getModuleParams($data->module));
	...
	public function getModuleParams($module) {
		...
		$path	= JPath::clean($client->path . '/modules/' . $module . '/' . $module . '.xml');
	...
	public function getModuleForm($module, $id = null) {
		$form = $this->getModuleParams($module);
		if ($id) {
	...
	private function convertModuleParams($form) {
	...
	private function addmodulemenu($id) {
		$db = JFactory::getDbo();
	...
	private function removemodulemenu($id) {
		// $db = JFactory::getDbo();
