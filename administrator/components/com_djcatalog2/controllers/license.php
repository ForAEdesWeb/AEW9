<?php
/**
 * @version $Id: license.php 326 2014-07-25 09:12:05Z michal $
 * @package DJ-Catalog2
...
class Djcatalog2ControllerLicense extends JControllerLegacy {
	...
	public function edit(){
		...
		echo '<div class="lic_manager">';
			echo '<form action="index.php" method="post" name="adminForm" id="adminForm" >'
				...
				.'<div class="lic_drow"><span class="lic_drow_label">'.JText::_('COM_DJCATALOG2_DJLIC_PRODUCT').'</span><code>'.$product.'</code></div>'
				.'<div class="lic_drow"><span class="lic_drow_label">'.JText::_('COM_DJCATALOG2_DJLIC_DOMAIN').'</span><code>'.$u->getHost().'</code></div>';
		
			
			echo '<div class="lic_drow"><span class="lic_drow_label">'.JText::_('COM_DJCATALOG2_DJLIC_LICENSE').'</span>';	
				if(!$key){
			...
			echo '<div class="lic_drow lic_status_row'.$cl.'"><span class="lic_drow_label">'.JText::_('COM_DJCATALOG2_DJLIC_STATUS').'</span>';
				
					if(!$key){
						echo '<div class="lic_status_box lic_invalid">';
							echo '<div class="lic_srow1">'.JText::_('COM_DJCATALOG2_DJLIC_ENTER_LICENSE').'</div>';
							echo '<div class="lic_srow2">'.JText::_('COM_DJCATALOG2_DJLIC_YOU_CAN_GET').' <a target="_blank" href="http://www.dj-extensions.com">DJ-EXTENSIONS.COM</a></div>';
						echo '</div>';
						...
						echo '<div class="lic_status_box lic_valid">';
							echo  '<div class="lic_srow1">'.JText::_('COM_DJCATALOG2_DJLIC_YOUR_LIC_IS_VALID').'</div>';											
							echo  '<div class="lic_srow2">'.JText::_('COM_DJCATALOG2_DJLIC_EXP_DATE').' '.date("d.m.y", strtotime($license[2])).'</div>';	
						echo '</div>';
					}
			...
			if($key){
				echo '<div class="lic_drow last"><span class="lic_drow_label">&nbsp;</span><input type="checkbox" name="release" value="1" /> '.JText::_('COM_DJCATALOG2_DJLIC_RELEASE_DOMAIN').'</div>';
			}
	...
	function update_list(){
		...
		function make_update3(ext,license,version){
			...
			$('update_link_'+ext+'').set('html','".JText::_('COM_DJCATALOG2_DJLIC_PLEASE_WAIT')." <img src=\"".JURI::base()."components/".$app->input->get('option', null, 'cmd')."/assets/images/loading.gif\" />');
			...
			var myRequest = new Request({
				...
				onRequest: function(){
					myElement.set('html', '<div style=\"text-align:center;\"><img style=\"margin-top:10px;\" src=\"".JURI::base()."components/".$app->input->get('option', null, 'cmd')."/assets/images/long_loader.gif\" /><br />".JText::_('COM_DJCATALOG2_DJLIC_LOADING')."</div>');
				},
				onSuccess: function(responseText){
					...
					if(res.getElement('.error, .alert-error')){
						$('update_link_'+ext+'').set('html','".JText::_('COM_DJCATALOG2_DJLIC_FAILED')."');
					} else {
						$('update_link_'+ext+'').set('html','".JText::_('COM_DJCATALOG2_DJLIC_DONE')." <img src=\"".JURI::base()."templates/hathor/images/admin/tick.png\" />');
					}
				...
				foreach($ext_versions as $r){
								...
								echo '<tr><td>'.JText::_($ext->name).'</td>';
									...
									if($e[2]){
										if(version_compare($c_version,$l_version,'<')){
											echo '<div id="update_link_'.$e[0].'" style="width:100px;"><span class="btn btn-mini btn-danger" '.$style.' onclick="make_update3(\''.$e[0].'\',\''.$license.'\',\''.$c_version.'\');">'.JText::_('COM_DJCATALOG2_DJLIC_UPDATE').'</span></div>';
										}else{
											echo '<span>'.JText::_('COM_DJCATALOG2_DJLIC_LATEST').' <img src="'.JURI::base().'templates/hathor/images/admin/tick.png" style="vertical-align: middle;" /></span>';
										}		
									}else{
										if($e[3]){
					...
					if(!$exist){
						...
						echo '<td>'.$e[0].'</td><td>---</td><td>'.$e[1].'</td><td>';
							if($e[2]){
								echo '<div id="update_link_'.$e[0].'" style="width:100px;"><span class="btn btn-mini btn-success" '.$style.' onclick="make_update3(\''.$e[0].'\',\''.$license.'\',\'0\');">'.JText::_('COM_DJCATALOG2_DJLIC_INSTALL').'</span></div>';
							}else{
								if($e[4]){
