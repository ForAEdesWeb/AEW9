...
if( JVERSION >= 3 ) {
    if ($saveOrder)
    {
        ...
        $saveOrderingUrl = 'index.php?option=com_webgallery&task=items.'.$method.'&tmpl=component';
...
<table class="table table-striped adminlist" id="itemList">
    <thead>
        <tr>
            ...
            <th class="center" width="5%">
                <?php echo JText::_('COM_WEBGALLERY_THUMB'); ?>
            </th>
    ...
    <?php foreach ($this->items as $i => $item) :
        ...
        $canEdit    = $user->authorise('core.edit',         'com_webgallery.item.'.$item->a_id);
        $canCheckin = $user->authorise('core.edit.state',   'com_webgallery.item.'.$item->a_id) || $item->get('a_checked_out') == $userId || $item->get('a_checked_out') == 0;
        $canChange  = $user->authorise('core.edit.state',   'com_webgallery.item.'.$item->a_id) && $canCheckin;
        $canEditOwn = $user->authorise('core.edit.own',     'com_webgallery.item.'.$item->a_id) && $item->get('a_created_by') == $userId;
        ...
        <?php endif; ?>
            ...
            <td class="center">
                <?php
                    ...
                    $src = JArrayHelper::getValue($thumbs, 0);
                    $src = $src ? JURI::root() . $item->thumbObject->getPath() . '/' . $src : "http://placehold.it/{$width}x{$height}";
                ?>
                <img src="<?php echo $src; ?>" alt="Thumb" width="<?php echo $width; ?>" height="<?php echo $height; ?>"  />
            </td>
            ...
            >
                ...
                <div class="item-title" >
                    ...
                    <?php if ($item->get('a_checked_out')) : ?>
                        <?php echo JHtml::_('jgrid.checkedout', $i, $item->get('c_name'), $item->get('a_checked_out_time'), 'items.', $canCheckin); ?>
                    <?php endif; ?>
                    
                    
                    <!-- Title -->
                    <?php if ($canEdit || $canEditOwn) : ?>
                        <a class="quick-edit-content" href="<?php echo JRoute::_('index.php?option=com_webgallery&task=item.edit&id='.$item->a_id); ?>">
                ...
                <div class="pull-left">
                    <?php
                        ...
                        if($canEdit || $canEditOwn){
                            JHtml::_('dropdown.edit', $item->a_id, 'item.');
                        ...
                        if ($item->get('a_checked_out') && $canCheckin) :
                            JHtml::_('dropdown.checkin', 'cb' . $i, 'items.');
                        endif;
                        
                        if($canChange || $canEditOwn) {
                            if ($trashed) :
                                JHtml::_('dropdown.untrash', 'cb' . $i, 'items.');
                            else :
            ...
            <!--PUBLISHED-->
            <td class="center">
                <?php echo JHtml::_('jgrid.published', $item->a_published, $i, 'items.', $canChange, 'cb', $item->get('a_publish_up'), $item->get('a_publish_down')); ?>
            </td>
