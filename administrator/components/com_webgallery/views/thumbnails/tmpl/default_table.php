...
if( JVERSION >= 3 ) {
    if ($saveOrder)
    {
        ...
        $saveOrderingUrl = 'index.php?option=com_webgallery&task=thumbnails.'.$method.'&tmpl=component';
...
<table class="table table-striped adminlist" id="itemList">
    <thead>
        <tr>
            ...
            <th width="10%"  class="center">
                <?php echo JHtml::_('grid.sort',  'PATH', 'a.path', $listDirn, $listOrder); ?>
            </th>
            ...
            <!--MODIFIED-->
            <th width="10%" class="center">
                <?php echo JHtml::_('grid.sort',  'COM_WEBGALLERY_MODIFIED', 'a.modified', $listDirn, $listOrder); ?>
            </th>
            
            <!--SCANNED-->
            <th width="10%" class="center">
                <?php echo JHtml::_('grid.sort',  'COM_WEBGALLERY_SCANNED', 'a.scanned', $listDirn, $listOrder); ?>
            </th>
    ...
    <?php foreach ($this->items as $i => $item) :
        ...
        $canEdit    = $user->authorise('core.edit',         'com_webgallery.thumbnail.'.$item->a_id);
        $canCheckin = $user->authorise('core.edit.state',   'com_webgallery.thumbnail.'.$item->a_id) || $item->get('a_checked_out') == $userId || $item->get('a_checked_out') == 0;
        $canChange  = $user->authorise('core.edit.state',   'com_webgallery.thumbnail.'.$item->a_id) && $canCheckin;
        $canEditOwn = $user->authorise('core.edit.own',     'com_webgallery.thumbnail.'.$item->a_id) && $item->get('a_created_by') == $userId;
        ...
        <?php if( JVERSION >= 3 ): ?>
            ...
            <?php if ($canChange) :
                ...
                <input type="text" style="display:none" name="order[]" size="5" value="<?php echo $nested ? $orderkey + 1 : $item->a_id;?>" class="width-20 text-area-order " />
            <?php else : ?>
        ...
        <?php endif; ?>
            ...
            >
                ...
                <div class="item-title" >
                    ...
                    <?php if ($item->get('a_checked_out')) : ?>
                        <?php echo JHtml::_('jgrid.checkedout', $i, $item->get('c_name'), $item->get('a_checked_out_time'), 'thumbnails.', $canCheckin); ?>
                    <?php endif; ?>
                    
                    
                    <!-- Title -->
                    <?php if ($canEdit || $canEditOwn) : ?>
                        <a class="quick-edit-content" href="<?php echo JRoute::_('index.php?option=com_webgallery&task=thumbnail.edit&id='.$item->a_id); ?>">
                ...
                <div class="pull-left">
                    <?php
                        ...
                        if($canEdit || $canEditOwn){
                            JHtml::_('dropdown.edit', $item->a_id, 'thumbnail.');
                        ...
                        if($canChange || $canEditOwn) {
                            if ($item->get('a_published')) :
                                JHtml::_('dropdown.unpublish', 'cb' . $i, 'thumbnails.');
                            else :
                                JHtml::_('dropdown.publish', 'cb' . $i, 'thumbnails.');
                            endif;
                                JHtml::_('dropdown.divider');
                        }
                        ...
                        if ($item->get('a_checked_out') && $canCheckin) :
                            JHtml::_('dropdown.checkin', 'cb' . $i, 'thumbnails.');
                        endif;
                        
                        if($canChange || $canEditOwn) {
                            if ($trashed) :
                                JHtml::_('dropdown.untrash', 'cb' . $i, 'thumbnails.');
                            else :
                                JHtml::_('dropdown.trash', 'cb' . $i, 'thumbnails.');
                            endif;
                        }
            ...
            <!--MODIFIED-->
            <td class="center">
                <?php echo JHtml::_('date', $item->get('a_modified'), JText::_('DATE_FORMAT_LC4')); ?>
            </td>
            
            <!--SCANNED-->
            <td class="center">
                <?php echo JHtml::_('jgrid.published', $item->a_scanned, $i, 'thumbnails.'); ?>
            </td>
